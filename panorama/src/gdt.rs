...
pub fn write_idt_value(
	...
	mem.inner().write_obj(val, BOOT_IDT_ADDR)?;
	Ok(())
}
...
pub fn write_gdt_table(
	...
	for (index, entry) in table.iter().enumerate() {
		let addr = mem
			...
			.ok_or(PanoramaError::InvalidGdtAddr)?;
		mem.inner().write_obj(*entry, addr)?;
	}
	Ok(())
}
...
fn get_limit(entry: u64) -> u32 {
	let limit: u32 = ((((entry) & 0x000F_0000_0000_0000) >> 32)
