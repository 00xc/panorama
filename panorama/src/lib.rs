...
use crate::devices::{
	block::{
		BlockConfig, BlockDevice, BlockDeviceSnapshot, BlockHandler,
		BlockType,
	},
...
impl Vmm {
	pub fn new<P: AsRef<Path>>(
		...
		let allocator =
			AddressAllocator::new(MMIO_GAP_START, MMIO_GAP_SIZE)?;
	...
	fn early_init<P: AsRef<Path>>(
		...
		let entry_loc = self.syms.location(
			bootinfo.entry.raw_value() + 0xffffffff80000000,
		);
		println!(
			"Entry point: {:#x}: {}",
	...
	/// cmdline.
	fn register_devices<P: AsRef<Path>>(
		&mut self,
		disk: P,
	) -> Result<(), PanoramaError> {
		// Serial console
	...
	fn register_block_device(
		...
		let mmio_range =
			self.register_mmio(BLOCK_IRQ, 0x1000, block.clone())?;
		...
		let notify_addr =
			mmio_range.base().0 + VIRTIO_MMIO_QUEUE_NOTIFY as u64;
			...
			let exit = loop {
				...
				let (exit, in_guest) = crate::time_it!({
					...
					let mut timer = self
						.guest_state
						.as_mut()
						.and_then(GuestState::take_timer);
					/*if timer.is_some() {
