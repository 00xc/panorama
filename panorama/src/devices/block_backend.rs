...
impl DirtyFile {
	...
	pub(super) fn snapshot(
		&mut self,
	) -> Result<DirtyFileSnapshot, PanoramaError> {
...
impl WriteVolatile for DirtyFile {
	fn write_volatile<B: BitmapSlice>(
		...
		let nwritten = WriteVolatile::write_volatile(
			&mut &mut self.map.get_mut()[pos..],
			buf,
		)?;
...
impl vmm_sys_util::write_zeroes::PunchHole for DirtyFile {
	fn punch_hole(
		...
		let Some((offset, length)) =
			usize::try_from(offset).ok().zip(length)
		else {
			return Ok(());
		};
...
mod test {
	...
	fn test_dirty() {
		...
		let mut vs = unsafe {
			VolatileSlice::new(buf.as_mut_ptr(), size_of_val(&buf))
		};
		...
		// Check the dirty map
		let expected_dirty =
			RangeSet::<[RangeInclusive<usize>; 2]>::from_ranges(&[
				0..=31,
				64..=95,
			]);
